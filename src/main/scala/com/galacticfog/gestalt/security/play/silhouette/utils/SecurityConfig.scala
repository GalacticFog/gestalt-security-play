...
object SecurityConfig {
  ...
  def mkString(path: String) = scala.io.Source.fromFile(path).mkString
  ...
  def resolveGestaltConfig(config: String): Try[String] = {
    if (!exists(config))
  ...
  def resolveGestaltHome(home: String): Try[String] = {
    import Res._
